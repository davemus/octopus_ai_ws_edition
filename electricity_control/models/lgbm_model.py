...
def prepare_df(data, weekday_mean_data, hour_average, target_column='Global_active_power', lag_start=1, lag_end=7):
    ...
    data[f"{target_column}_hour_average"] = data['hour'].apply(lambda x: hour_average[x])
...
def notebook_prepare_data(all_data, new_data, w_m_d, h_a):
    ...
    if all_data.shape[0] >= 6:
        data = prepare_df(all_data, w_m_d, h_a)
...
class LGBMModel(Model):
    ...
    def __init__(self, path_to_weights, path_to_data, prepare_data_function=notebook_prepare_data):
        self.path_to_weights = path_to_weights
        self.path_to_data = path_to_data
        self.weekday_mean_data, self.hour_average = None, None
    ...
    def predict(self, data):
        ...
        self.all_data, data = self.prepare_data(self.all_data, data,
                                                self.weekday_mean_data, self.hour_average)
        if data is not None:
